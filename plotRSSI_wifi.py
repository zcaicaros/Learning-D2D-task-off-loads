...
Itervalrecord_uWIFIrssi = [0 for i in range(len(timeintervals)-1)]
...
for n in fname[:]:
    ...
    g = subprocess.Popen(['grep','-E', 'wifi\|connected.*rssi', '/usr/666Users/'+n], stdout = subprocess.PIPE) # subproess grep the file and retrn a file object stored in g
    lines = g.stdout.readlines()
    for i in range(len(lines)):
        try:
            wifi_rssimatch = re.findall(r"rssi\;(.+?)$", lines[i])
            lv = int(wifi_rssimatch[0])
#            print lv
 ...
 #           print ('can\'t find time 00:00:00 in line: ', i )# apply error handling
    ...
    print '\n'
    print n + " has loaded and current wifiRSSI is : "
    print accvalues_interval
    

h = open('/usr/666Users/pythonprograms/fun/data/666/WIFIRSSI/666wifiRSSI_no4digits.txt', 'w')
for num in accvalues_interval:
    h.write("%s\n" % num)
e = open('/usr/666Users/pythonprograms/fun/data/666/WIFIRSSI/Dividor_666wifiRSSI_no4digits.txt', 'w')
for num in Itervalrecord_uWIFIrssi:
    e.write("%s\n" % num)
